...
export default function ThaidConfirmPinPage() {
  ...
  useEffect(() => {
    if (pin.length === maxPin) {
      setTimeout(() => router.push('/form'), 300)
    }
  }, [pin, router])
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ background: '#1e3080', height: 56, display: 'flex', alignItems: 'center', padding: '0 16px', gap: 10, flexShrink: 0 }}>
        <span style={{ color: '#fff', fontSize: 24, cursor: 'pointer' }} onClick={() => router.back()}>‹</span>
        <span style={{ color: '#fff', fontSize: 16, fontWeight: 600 }}>ยืนยันตัวตน</span>
      </div>
