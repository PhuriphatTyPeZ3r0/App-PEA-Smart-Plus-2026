...
export default function PinConfirmPage() {
  ...
  useEffect(() => {
    if (pin.length === maxPin) {
      // Set hasSeenSplash so HomePage doesn't redirect back to splash
      sessionStorage.setItem("hasSeenSplash", "true")
      setTimeout(() => router.push('/biometric'), 400)
    }
  }, [pin, router])
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', alignItems: 'center' }}>
      ...
      <p style={{ fontSize: 16, fontWeight: 600, color: error ? '#FF3B30' : '#333', flexShrink: 0, margin: '12px 0 0' }}>
        {error ? 'รหัส PIN ไม่ตรงกัน กรุณาลองใหม่' : 'ยืนยันรหัส 6 หลัก อีกครั้ง'}
      </p>
        ...
        {Array.from({ length: maxPin }).map((_, i) => (
          <div key={i}
            style={{
              ...
              border: '2px solid',
              borderColor: error ? '#FF3B30' : i < pin.length ? '#8B1A6B' : '#CCC',
              background: error ? '#FF3B30' : i < pin.length ? '#8B1A6B' : '#fff',
              transition: 'all .15s'
            }}
          />
        ))}
